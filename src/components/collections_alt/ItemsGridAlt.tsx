...
const items: Item[] = [
  { images: [imgA, imgB, imgC, imgD], name: 'Abito Aurora', badge: 'Serie limitata', price: 'Su richiesta' },
...
function ItemCard({ item }: { item: Item }) {
  ...
  return (
    <article className="group border border-gray-100 bg-transparent shadow-[0_4px_16px_rgba(0,0,0,0.04)] hover:shadow-[0_8px_24px_rgba(0,0,0,0.06)] transition-shadow">
      <div className="relative aspect-[3/4] w-full overflow-hidden bg-[#f6f1f2]">
        <img
          src={activeImage}
        ...
        <div className="absolute inset-x-0 bottom-0">
          <div className="relative px-3 py-2 md:px-4 md:py-3">
            ...
            <div className="relative z-30 flex items-center justify-between gap-3">
              <div>
                <h3 className="font-serif text-white text-base md:text-lg leading-tight">{item.name}</h3>
                <div className="mt-0.5 text-xs md:text-sm text-white/85">{item.price || 'Su richiesta'}</div>
              </div>
              <Link
                to="/collezione"
                className="shrink-0 inline-flex items-center justify-center px-2.5 py-1 border border-[#E8BFC7] text-[#6B4A4E] bg-[#F6D4D8]/90 hover:bg-[#F3C9CF] rounded-full text-xs tracking-wide transition-colors duration-200"
              >
                Dettagli
              </Link>
            </div>
      ...
      <div className="px-3 md:px-4 py-2 flex items-center gap-2 border-t border-gray-100 bg-white/40">
        {item.images.slice(0, 4).map((thumb, i) => (
          <button
            ...
            className={`relative w-10 h-10 rounded-sm overflow-hidden border ${activeIdx === i ? 'border-[#E8BFC7] ring-1 ring-[#E8BFC7]' : 'border-gray-200'} hover:border-[#E8BFC7] transition-colors`}
          >
